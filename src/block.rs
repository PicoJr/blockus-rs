...
impl Block {
    pub fn from_str(s: &str) -> Result<Self, BlockError> {
        ...
        Ok(Block {
            data: DMatrix::from_row_iterator(rows.len(), max_columns, rows_flattened),
        })
    }
    ...
    pub fn rotate_90(&self) -> Block {
        ...
        let rows: Vec<Vec<bool>> = transposed
            .row_iter()
            .map(|r| {
...
mod test {
    ...
    fn test_rotate_9() {
        ...
        assert_eq!(
            b.rotate_90().rotate_90(),
            Block::from_str("###\n  #").unwrap()
        );
        assert_eq!(
            b.rotate_90().rotate_90().rotate_90(),
            Block::from_str(" #\n #\n##").unwrap()
        );
    }
    #[test]
